...
if MODEL_TYPE == 'PDE' and TEST_CASE == 'multi_phase_pipeflow_with_leak':
    ...
    for test_case_index in TEST_CASE_INDEX_LIST:
        for num_particles in NUM_PARTICLES_LIST:
            for observation_time_interval in OBSERVATION_TIME_INTERVAL_LIST:
                ...
                if TEST_CASE == 'wave_submerged_bar':
                    config['observation_args']['observation_times'] = [100, 1000, observation_time_interval]
                ...
                if TEST_CASE == 'wave_submerged_bar':
                    LOCAL_SAVE_PATH = f'{LOCAL_SAVE_PATH}_obs_time_{observation_time_interval}'
                    ORACLE_SAVE_PATH = f'{ORACLE_SAVE_PATH}_obs_time_{observation_time_interval}'
                ...
                if config['model_type'] in ['latent', 'latent_WAE_conv', 'latent_AE_no_reg']:
                ...
                RMSE = np.mean((mean_pred - true_solution.state[:, :, true_solution.observation_times[-1]])**2)
                RMSE = np.sqrt(RMSE)
                print(f'RMSE: {RMSE}')
